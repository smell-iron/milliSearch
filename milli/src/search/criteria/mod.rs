    ...
    match (&left.kind, &right.kind) {
        (QueryKind::Exact { word: left, .. }, QueryKind::Exact { word: right, .. }) => {
            if prefix {
                ...
                //    proximity ranking rule for the prefixes of the right word. This is done by:
                //    1. Only find the documents for the exact (ie non-prefix) right word
                ...
                match (
                    ...
                    (true, true, true) => Ok(word_prefix_pair_overall_proximity_docids(
                        ctx,
                        left.as_str(),
                        right.as_str(),
                        proximity,
